...
Discogs.search = (params) => {
  return axios.get(`https://api.discogs.com/database/search?`, { params })
    .then(res => res.data)
...
Discogs.createVinyl = (params) => {
  return Promise.all([Discogs.getTracklist(params), Discogs.getGenres(params), Discogs.getLabels(params)])
  .then(res => {
    return {
      title: params.title,
      ...
      catno: res[2][0].catno //Take catalog number from first label entery
    }
  })
  .catch(err => console.error(err));
}
